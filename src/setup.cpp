...
int VER_SEPAR = (int)(HOR_SEPAR / 2 * 1.73);
int ARENA_WIDTH = COLS * HOR_SEPAR;
int ARENA_HEIGHT = (ROWS - 1) * VER_SEPAR;
// int iteration = 0;

// function to check if randomly generated positions are in collision
bool collision(float x, float y, float pos_list[ROBOT_COUNT + BEACON_NUM][4], int limit) {
...
void setup_positions(float robot_pos[ROBOT_COUNT + BEACON_NUM][4])
{
...
//assign each robot a random position, centered around light source
	...
	for (int i = COLS; i < BEACON_NUM - 2*COLS - 1 + (ROWS%2); i++){
		int c = (i-COLS) % 2;
		int r = (i-COLS) / 2;
...
// triangular positions
	...
	// robot_pos[3][2] = 0;
	// robot_pos[3][3] = 2;
	for (int i = BEACON_NUM; i < BEACON_NUM + ROBOT_COUNT; i++) {
		float x0;
		float y0;
		float t0;
		do
		...
		} while (collision(x0, y0, robot_pos, BEACON_NUM));
		robot_pos[0][0] = x0;
		robot_pos[0][1] = y0;
		robot_pos[0][2] = t0;
		robot_pos[0][3] = i;
